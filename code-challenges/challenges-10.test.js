...
const count = (target, input) => {
  // Solution code here...
  let e=0;
let x= input.map(function(it){
let y= it.map(function(it){
if(it==target){
  e++;
}
...
let y= it.reduce(function(sum,it){
...
let z= x.reduce(function(sum,it){
...
let y= it.filter(function(it){
 
if(typeof(it ) == 'number' && it % 5 === 0  ){

return true
}else{return false} 

})
let z = y.map(function(it){
...
let res=arr.toString().replace(/\,\b/g, " and " );
return res
};
...
let findShortest = (data) => {
  // Solution code here...
  let heights=data.map(function(it){
let i =parseInt(it.height)
    return i
  })

  let res=data.filter(function(it){

let heightItem=parseInt(it.height)

function trueORfalse(){

let trigger=false;
for(let xy=0;xy<heights.length;xy++){

  if(heights[xy]>heightItem){
trigger=true
}else{trigger=false}
}
return trigger
